        ...
        def Increment(self):
            while True:
                ...
                if self.first== True:
                    ...
                    self.calendar_message = self.channel.send(embed=calanderembed)
                    self.first=False
                    time.sleep(4.165)
                else:
                    calanderembed=discord.Embed(title="Hypixel Calander", color=0x808080)
                    ...
                    calanderembed.add_field(name="Current Major Events", value="CurrentEvents", inline=True)
                    calanderembed.add_field(name="Major Events soon", value="EventsSoon", inline=True)
                    self.calendar_message.edit(embed=calanderembed)
                    time.sleep(4.165)
        Incrementer= threading.Thread(target=Increment)
        Incrementer.start()
    async def calenderinit(self):
        def calandercheck(self):
            while True:
                utc_now = datetime.now(timezone.utc)
                skyblockstart = datetime.strptime("2019-06-11 17:55:00", "%Y-%m-%d %H:%M:%S").replace(tzinfo=timezone.utc)
                #IRL time
                ...
                if int(round(self.IG_minute)) % 5 == 0:
                    ...
                    initialiser.join()
                    break
        initialiser= threading.Thread(target=calandercheck)
        initialiser.start()
